...
- `0.10.213`: **CI — Fly deploy workflows & token** — Job-level `FLY_API_TOKEN`; fail-fast step if unset; browser worker deploy from repo root: `flyctl deploy apps/browser-worker --config apps/browser-worker/fly.toml`. (Fork PRs cannot use repo secrets — use manual deploy or merge to default branch.)
