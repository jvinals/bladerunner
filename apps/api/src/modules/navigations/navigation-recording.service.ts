...
export class NavigationRecordingService {
  ...
  /**
   * Ephemeral pointer move: positions the Playwright cursor for CSS :hover, tooltips, and
   * popovers. Not persisted as a NavigationAction (same category as scroll).
   */
  async pointerMove(
    navId: string,
    userId: string,
    ...
    const { px, py } = this.streamToViewportCss(session, streamX, streamY, streamWidth, streamHeight);
  ...
  private async requestBrowserFromWorker(workerUrl: string, navId: string): Promise<string> {
    ...
    for (let i = 1; i <= WORKER_WS_ATTEMPTS; i++) {
      try {
        ...
        this.logger.warn(
          `Browser worker WS failed (${warnDetail}); retry ${i}/${WORKER_WS_ATTEMPTS} in ${backoff}ms`,
        );
