...
- **Tables — row by position (first / second / last / Nth):** When the user asks to click **the first row**, **second row**, **last row**, or similar, **do not** fabricate \`getByRole('row', { name: /…/i })\` from **OCR of IDs, MRNs, or small table text** in the screenshot — digits are easily misread at any resolution. Prefer **structural** locators: \`page.locator('tbody tr').first()\`, \`page.getByRole('table').locator('tbody tr').nth(N)\`, or \`page.getByRole('table').getByRole('row').nth(N)\` (if row 0 is a header with \`role="row"\`, use \`nth(1)\` for the first **data** row). For **last** data row, \`page.locator('tbody tr').last()\` or the last \`getByRole('row')\` inside the table body. Use \`getByRole('row', { name: /…/i })\` **only** when the instruction or **accessibility tree** names a **specific** stable string, not a guessed code from the image.
- **Tables (strict mode — cells):** Never use \`locator('table tr:last-child td')\` or \`locator('tr:last-child td')\` **without** scoping to **one** cell — that selector matches **every** \`<td>\` in the row. Prefer \`getByRole('row', { name: /unique name/i }).getByRole('cell').first()\` **only when the row is identified safely** (see above), \`locator('tr:last-child').locator('td').nth(0)\`, or target a **row action button** (e.g. \`getByRole('button', { name: 'Open' })\` scoped to the row). If you must click the first cell, use \`.locator('td').first()\` **on the row locator**, not \`tr:last-child td\` alone.
