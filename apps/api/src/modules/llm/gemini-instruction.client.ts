...
For search fields and comboboxes, use getByPlaceholder when the UI shows placeholder text, or getByRole('combobox') / getByRole('textbox'), not locator('xpath=following::input') after a label: the first following input is often a hidden type=file upload; excluding it in XPath can match nothing and cause long waits. If you must chain from visible text, scope to the same card/section with a stable parent selector first.
